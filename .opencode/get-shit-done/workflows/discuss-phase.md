...
```bash
INIT=$(node "/home/tom/workspace/ai/made/workspace/askx/.opencode/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE}")
...
**Auto mode (`--auto`):** For each area, Claude selects the recommended option (first option, or the one explicitly marked "recommended") for every question without using question. Log each auto-selected choice:
```
...
```bash
node "/home/tom/workspace/ai/made/workspace/askx/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
node "/home/tom/workspace/ai/made/workspace/askx/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
